...
import { athletes as staticAthletes } from '../data/dashboardData';
import { fetchTeamSummary, fetchAthleteData } from '../utils/dataService';
...
export default function Reports() {
    ...
    const handleAction = async (athlete, autoDownload = false) => {
        ...
        if (!data) {
            ...
            const fullAthlete = staticAthletes.find(a => a.id === athlete.id);
            data = await fetchAthleteData(fullAthlete);
                            ...
                            <tbody className="divide-y divide-border/50">
                                {filteredAthletes.length > 0 ? (
                                    filteredAthletes.map((athlete) => {
                                        ...
                                        const staticInfo = staticAthletes.find(a => a.id === athlete.id);
                                        ...
                                        return (
                                            <tr key={athlete.id} className="group hover:bg-secondary/20 transition-all">
                                                ...
                                                <td className="px-6 py-5">
                                                    <span className="text-lg font-bold text-foreground">{athlete.avgHR.toFixed(1)} <small className="text-xs text-muted-foreground opacity-60">BPM</small></span>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <span className="text-lg font-bold text-foreground">{athlete.rmssd.toFixed(1)} <small className="text-xs text-muted-foreground opacity-60">MS</small></span>
                                                </td>
                                                <td className="px-6 py-5">
